...
export default function EditServicePage({
  ...
  const fetchService = async () => {
    try {
      ...
      setFormData({
        ...
        defaultHours: data.defaultHours?.toString() || "",
        isFlatRate: data.isFlatRate !== undefined ? data.isFlatRate : true,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(`/parts-services/services/${id}`)
    } catch (err) {
    ...
    <div className="space-y-6 p-4 sm:p-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <Button variant="ghost" size="icon" asChild className="self-start">
          <Link href={`/parts-services/services/${id}`}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-4">
          <Button 
            type="button" 
            variant="outline" 
            onClick={() => router.push(`/parts-services/services/${id}`)}
